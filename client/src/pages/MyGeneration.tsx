...
const MyGeneration = () => {
  ...
  const handleDownload = (image_url: string) => {
    ...
    link.download = `${image_url.split('/').pop()}`;
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      console.log(error);
      toast.error(error?.response?.data?.message || 'Failed to Delete thumbnails')
    }
            ...
            {thumbnails.map((thumb: IThumbnail) => {
              ...
              return (
                <div
                  ...
                  className="mb-8 group relative cursor-pointer rounded-2xl bg-white/6 border border-white/10 shadow-xl transition break-inside-avoid"
                >
                  {/* IMAGE */}
                  <div
                  ...
                  <div className="p-4 space-y-2">
                    ...
                    <p className="text-xs text-zinc-500">
                      {thumb.createdAt
                        ? new Date(thumb.createdAt).toLocaleDateString("en-IN", {
                          day: "numeric",
                          month: "short",
                          year: "numeric",
                        })
